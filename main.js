...
async function run() {
    ...
    q.question(colors.brightGreen('Which presence do you want to run?\nRespond with the number that corresponds with the status\nyou would like to use. ---->'), async rpc => {
        if (!rpcs[rpc - 1]) {
            if (isNaN(rpc)) {
                logger.error(`You might want to respond using a number`);
                return q.close();
            }
        ...
        try {
            ...
            logger.error(`Error occurred while trying to load ./presences/${rpcs[rpc - 1]}\nError:${e}`);
        }
        q.close();
    });
}; run();
